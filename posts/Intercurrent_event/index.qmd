...
First, for the true value of the per-protocol method $\tau_{pp}$, we generate a large simulated data with sample size $10^6$ and then calculate the true value of $\tau_{pp}$ as: 
$$\tau_{pp}^{true} = \widehat{E}\{Y|Z=1,S=1\} - \widehat{E}\{Y|Z=0,S=1\}.$$
